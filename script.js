(function() {
  ...
  const minutsLI = document.querySelector("#minuts")
  const secundsLI = document.querySelector("#secunds")

  function timeCountDwon (){
    const newdata = new Date();
    const newyeardata =new Date(newYear);
    const totolSecunds  = (newyeardata - newdata) /1000;

    const days = Math.floor(totolSecunds / 3600 / 24);
    const hours = Math.floor(totolSecunds / 3600) % 24;
    const minuts = Math.floor(totolSecunds / 60) % 60;
    const secunds = Math.floor(totolSecunds) % 60;

    daysLI.textContent = formatTime(days)
    hoursLI.textContent = formatTime(hours)
    minutsLI.textContent = formatTime(minuts)
    secundsLI.textContent = formatTime(secunds)
  }
  ...
  timeCountDwon()
  setInterval(timeCountDwon, 1000)